...
func formatKVLine(fields map[string]string) string {
	keys := []string{"ts", "level", "event", "run", "kive", "generation", "seq", "worker", "job", "phase", "action", "cmd", "stream", "exit", "duration_ms", "reason", "msg"}
...
func formatJSONLine(fields map[string]string) string {
	...
	for key, value := range fields {
		...
		switch key {
		case "generation", "seq", "exit", "duration_ms":
...
func baseLogFields(run RunContext, workerIP string) map[string]string {
	...
	if run.KiveCmd != "" {
		fields["kive"] = run.KiveCmd
	}
	if run.Generation > 0 {
		fields["generation"] = strconv.Itoa(run.Generation)
	}
	if workerIP != "" {
...
// CLIStreamTarget returns the job@worker prefix used in live terminal output.
func CLIStreamTarget(workerIP, job string) string {
	return cliStreamTarget(workerIP, job)
}
