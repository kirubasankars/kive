...
// RunContext identifies one kive CLI invocation in logs.
...
type CommandContext struct {
	...
	Action string
	Cmd    string
	// Quiet suppresses live terminal stream lines (stdout/stderr still go to run logs).
	Quiet bool
}

// NewRunContext returns a fresh run context for kiveCmd.
func NewRunContext(kiveCmd string, generation int) RunContext {
	return RunContext{
		RunID:     newRunID(),
		KiveCmd:  kiveCmd,
