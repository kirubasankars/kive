...
func BuildCatalog(tx *sql.Tx, jobWorkspace workspace.Workspace) error {
	...
	for _, jobName := range jobNames {
		...
		vars, err := data.NewJobTemplateVars(tx, jobName)
		if err != nil {
			return err
		}
		for _, rel := range alertPaths {
			...
			_, contentStr, _ := workspace.MaterializeJobTemplate(filePath, string(content), vars)
...
func syncJobScrapeNameKV(jobName string, configs []map[string]interface{}) {
	...
	kv.GetKVStore().Put(
		fmt.Sprintf("kive/job/%s", jobName),
		"job_name",
...
func hasPrometheusServerJob(jobNames []string) bool {
	for _, jobName := range jobNames {
		if workspace.JobHasPrometheusServerConfigFile(jobName) {
			return true
		}
	}
	return false
