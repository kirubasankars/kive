...
// ErrNoActiveScrapeTargets is returned when kive:port placeholders expand to zero targets.
...
func ValidateScrapeConfigs(jobName string, configs []map[string]interface{}) error {
	for idx, cfg := range configs {
		for _, key := range forbiddenServiceDiscoveryKeys {
			if _, ok := cfg[key]; ok {
				return fmt.Errorf("%w: job %s scrape.yaml[%d] %s is not supported (use static_configs with kive:port placeholders)",
...
// ValidateScrapePortReferences checks kive:port placeholders against manifest ports at build time.
...
// ScrapeConfigsRequireActiveAllocations reports whether any static_configs target uses kive:port placeholders.
...
func ResolveMetricsPortKey(jobName string, ports workspace.ManifestPorts) (string, error) {
	...
	case 0:
		return "", fmt.Errorf("%w: job %s scrape.yaml uses kive:port/_implicit but no metrics port is declared in manifest",
...
// ResolveScrapeJobName expands kive:job to the kive job folder name.
func ResolveScrapeJobName(kiveJob, value string) string {
...
// ExpandScrapeConfigs resolves kive:port and kive:job placeholders using active allocations and assigned ports.
...
func expandScrapeJobReferences(kiveJob string, cfg map[string]interface{}) {
	if raw, ok := cfg["job_name"].(string); ok {
		cfg["job_name"] = ResolveScrapeJobName(kiveJob, raw)
	}
	...
	for _, block := range staticSlice {
		...
		for key, rawValue := range labelMap {
			...
			labelMap[key] = ResolveScrapeJobName(kiveJob, value)
		}
	}
}
...
func expandTarget(
	...
	portKey := parts[0]
	if portKey == "" {
		return nil, fmt.Errorf("%w: invalid kive port placeholder %q", bucket.ErrInvalidJob, target)
	}
	...
	if !ok {
		return nil, fmt.Errorf("%w: port %q has no assigned number (run kive build)", bucket.ErrInvalidJob, portKey)
	}
