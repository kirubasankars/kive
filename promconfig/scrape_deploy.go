...
// RenderScrapeConfigsYAML expands kive:port placeholders using live allocations and ports,
// then returns an indented YAML fragment for prometheus job templates.
// jobsFilter limits output to named kive jobs; nil or empty means all scrape jobs.
...
func expandScrapeConfigsForJob(tx *sql.Tx, jobName string, configs []map[string]interface{}) ([]map[string]interface{}, error) {
	...
	portNumbers, err := data.GetPortMapInt(tx, jobName)
	if err != nil {
		return nil, err
	}
