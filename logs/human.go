...
func humanHeader(entry Entry) string {
	...
	if kive := field(entry, "kive"); kive != "" {
		parts = append(parts, kive)
	}
	if generation := field(entry, "generation"); generation != "" {
		parts = append(parts, "generation="+generation)
	} else if seq := field(entry, "seq"); seq != "" {
		// Legacy log lines before generation rename.
...
func entryMatchesFilters(entry Entry, opts ShowOptions) bool {
	...
	if opts.KiveCmd != "" && field(entry, "kive") != opts.KiveCmd {
		return false
	}
	return true
}
