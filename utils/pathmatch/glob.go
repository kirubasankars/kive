...
// MatchNotAny reports paths from rels that match none of the patterns (sorted, unique).
func MatchNotAny(patterns, rels []string) []string {
	if len(patterns) == 0 || len(rels) == 0 {
		return nil
	}
	...
	for _, rel := range rels {
		rel = filepath.ToSlash(rel)
		if len(MatchAny(patterns, []string{rel})) > 0 {
			continue
		}
		if _, ok := seen[rel]; !ok {
			seen[rel] = struct{}{}
