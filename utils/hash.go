...
// FilterDirectoryTree returns a copy of tree without paths where skip(rel) is true.
func FilterDirectoryTree(tree DirectoryTree, skip func(string) bool) DirectoryTree {
	filtered := make(map[string]string, len(tree.Files))
	rels := make([]string, 0, len(tree.Files))
	for rel, digest := range tree.Files {
		if skip(rel) {
			continue
		}
		filtered[rel] = digest
	...
	for _, rel := range rels {
		if _, err := combined.Write([]byte(filtered[rel])); err != nil {
			return DirectoryTree{Aggregate: tree.Aggregate, Files: filtered}
		}
	}
	return DirectoryTree{
		Aggregate: hex.EncodeToString(combined.Sum(nil)),
