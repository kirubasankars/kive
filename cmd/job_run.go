...
var jobStopCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		workersComma, _ := flags.GetString("allocations")
		healthCheck, _ := flags.GetBool("health_check")
		forget, _ := flags.GetBool("forget")
		err := jobcontrol.ExecuteRequest(jobcontrol.Request{
			JobsCSV:     args[0],
			WorkersCSV:  workersComma,
			Target:      jobcontrol.TargetStop,
...
func init() {
	...
	jobStopCmd.Flags().Bool("forget", false, "after stop, clear promoted deploy state so the next deploy starts without pre-batch health (post-health still required)")
