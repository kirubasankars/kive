...
var logsShowCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		kiveCmd, _ := flags.GetString("kive")
...
func init() {
	...
	logsShowCmd.Flags().String("event", "", "filter by event (command_begin, deploy_skip, run_begin, ...)")
	logsShowCmd.Flags().String("kive", "", "filter by kive command (deploy, job, build, run_command, worker, gc)")
