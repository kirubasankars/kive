...
// prometheusServerConfigPaths returns job_files paths for prometheus.yml in every template form.
func prometheusServerConfigPaths(jobName string) []string {
	return []string{
		jobName + "/prometheus.yml",
		jobName + "/prometheus.yml.bt",
		jobName + "/prometheus.yml.tpl",
		jobName + "/prometheus.yml.tpl.bt",
	}
}

// JobHasPrometheusServerConfig reports whether the job ships prometheus.yml in any template form
// (plain, .bt, .tpl, or .tpl.bt).
func JobHasPrometheusServerConfig(tx *sql.Tx, jobName string) (bool, error) {
	paths := prometheusServerConfigPaths(jobName)
	...
	rows, err := tx.Query(
		`SELECT j.name, jf.path
		 ...
		 WHERE jf.isdir = 0 AND jf.path LIKE '%/_prometheus/alerts/%'
		   AND (
		     jf.path LIKE '%.yaml' OR jf.path LIKE '%.yml'
		     OR jf.path LIKE '%.yaml.bt' OR jf.path LIKE '%.yml.bt'
		   )
...
// ListDashboardPluginFiles returns in-app dashboard plugin paths (*.js) in the catalog.
func ListDashboardPluginFiles(tx *sql.Tx) ([]string, error) {
	rows, err := tx.Query(
		`SELECT path FROM job_files
		 WHERE isdir = 0 AND path LIKE '%/_prometheus/dashboards/%.js'
		 ORDER BY path`,
	)
	if err != nil {
		return nil, bucket.DatabaseError(err)
