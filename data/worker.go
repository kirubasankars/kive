...
func GetWorkers(tx *sql.Tx, labels []string) ([]string, error) {
	...
	if len(labels) == 0 {
		rows, err := tx.Query(`SELECT worker_ip FROM workers ORDER BY position`)
		if err != nil {
	...
	if len(labels) > 0 {
		...
		query := `SELECT DISTINCT worker_ip FROM workers w
			 JOIN worker_labels wl ON w.worker_id = wl.worker_id
			 WHERE label IN (` + placeholders + `) ORDER BY position`
...
func GetWorkerID(tx *sql.Tx, workerIP string) (string, error) {
	var workerID string
	row := tx.QueryRow("SELECT worker_id FROM workers WHERE worker_ip = ?", workerIP)
...
func GetAllWorkers(tx *sql.Tx) ([]string, error) {
	var workers []string
	rows, err := tx.Query("SELECT worker_ip FROM workers")
	if err != nil {
		return nil, bucket.DatabaseError(err)
	}
	defer func() {
...
func GetWorkerCPU(tx *sql.Tx, workerIP string) (string, error) {
	...
	row := tx.QueryRow("SELECT available_cpu_mhz FROM workers WHERE worker_ip = ?", workerIP)
...
func GetWorkerMemory(tx *sql.Tx, workerIP string) (string, error) {
	...
	row := tx.QueryRow("SELECT available_memory_mb FROM workers WHERE worker_ip = ?", workerIP)
...
func GetActiveAllocationsOrdered(tx *sql.Tx, job string) ([]string, error) {
	rows, err := tx.Query(`
		SELECT a.worker_ip
		FROM allocations a
		INNER JOIN workers w ON w.worker_ip = a.worker_ip
...
func GetWorkerPosition(tx *sql.Tx, workerIP string) (int, error) {
	var position int
	err := tx.QueryRow(`SELECT position FROM workers WHERE worker_ip = ?`, workerIP).Scan(&position)
	if err != nil {
