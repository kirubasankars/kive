...
func KV(jobsCSV string, activeOnly, deletedOnly bool) error {
	...
	count := 0
	row := tx.QueryRow("SELECT count(*) FROM (" + data.CatalogKVSQL + ") AS cat_kv" + where)
	err = row.Scan(&count)
	...
	rows, err := tx.Query(`SELECT namespace, key, value, version, ttl, created_date, deleted FROM (` + data.CatalogKVSQL + `) AS cat_kv` + where)
	if err != nil {
		return bucket.DatabaseError(err)
	}
	defer func() {
