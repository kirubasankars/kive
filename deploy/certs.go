...
func updateCerts(tx *sql.Tx, job, workerIP string) error {
	...
	rows, err := tx.Query(
		"SELECT name FROM job_certs WHERE job_id = (SELECT job_id FROM jobs WHERE name = ?)",
		job,
	)
	if err != nil {
	...
	workerKVNamespace := fmt.Sprintf("kive/job/%s/worker/%s", job, workerIP)
	...
	caTrust, err := store.Get("kive/worker", "certs/ca-trust.crt")
	if err != nil {
		return err
	}
	if err := os.WriteFile(path.Join(certsDir, "ca-trust.crt"), []byte(caTrust.Value), 0o644); err != nil {
		return err
	}
	_ = os.Remove(path.Join(certsDir, "ca.crt"))
	return nil
}
