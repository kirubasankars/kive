...
func ParseVersionConstraint(config map[string]interface{}) (VersionConstraint, error) {
	...
	if constraint.Min != nil && constraint.Max != nil && constraint.Min.Compare(*constraint.Max) > 0 {
		return constraint, fmt.Errorf("%w: min_version %s > max_version %s",
			bucket.ErrInvalidHookConfiguration, constraint.Min, constraint.Max)
	}
	return constraint, nil
}
...
// ValidateHookDemand checks demand job/hook pairing.
func ValidateHookDemand(jobName, hookName string, demand Hook) error {
	demandJob := strings.TrimSpace(demand.Demands.Job)
	demandHook := strings.TrimSpace(demand.Demands.Hook)

	switch {
	case demandJob == "" && demandHook == "":
		return nil
	case demandJob == "":
		return fmt.Errorf("%w: job %s hook %s demands.job must be set when demands.hook is set",
			bucket.ErrInvalidHookDemand, jobName, hookName)
	case demandJob == jobName:
		return fmt.Errorf("%w: job %s, hook %s invalid configuration, self referencing",
			bucket.ErrInvalidHookConfiguration, jobName, hookName)
	}
	return nil
}
...
func (c VersionConstraint) Satisfies(upstream Version) error {
	if c.Min != nil && upstream.Compare(*c.Min) < 0 {
		return fmt.Errorf("%w: upstream version %s below min_version %s",
			bucket.ErrHookDemandVersionMismatch, upstream, c.Min)
	}
	if c.Max != nil && upstream.Compare(*c.Max) > 0 {
		return fmt.Errorf("%w: upstream version %s above max_version %s",
			bucket.ErrHookDemandVersionMismatch, upstream, c.Max)
	}
	return nil
}
