...
const (
	...
	DefaultHealthCheckWaitAttempts        = 60
	DefaultHealthCheckWaitIntervalSeconds = 2
)
...
type ManifestHealthCheck struct {
	...
	Wait           *HealthCheckWait   `json:"wait,omitempty"` // default for kinds without local wait; Normalize fills 60/2s when omitted
}
...
// Returns nil when neither is set (caller uses bucket health.wait_seconds).
// After Normalize, outer wait is the built-in 60/2s default when job.conf omitted it.
...
// and clears Checks. When outer wait is omitted, sets 60 attempts at 2s.
// Does not set timeout_seconds. New job.conf must declare probes under
// liveness { ... } / readiness { ... }.
func (hc *ManifestHealthCheck) Normalize() {
	...
	if hc.Wait == nil {
		hc.Wait = &HealthCheckWait{
			Attempts:        DefaultHealthCheckWaitAttempts,
			IntervalSeconds: DefaultHealthCheckWaitIntervalSeconds,
		}
	}
}
