...
// ParseReloadGlobs decodes reload_globs stored on the job row.
func ParseReloadGlobs(raw string) ([]string, error) {
	raw = strings.TrimSpace(raw)
	if raw == "" || raw == "[]" {
		return nil, nil
	}
	var globs []string
	if err := json.Unmarshal([]byte(raw), &globs); err != nil {
		return nil, fmt.Errorf("%w: invalid reload_globs JSON: %w", bucket.ErrInvalidManifest, err)
	}
	return globs, nil
}

// EncodeReloadGlobs serializes reload_globs for the job row.
func EncodeReloadGlobs(globs []string) (string, error) {
	if len(globs) == 0 {
		return "[]", nil
	}
	encoded, err := json.Marshal(globs)
	if err != nil {
		return "", fmt.Errorf("%w: reload_globs: %w", bucket.ErrInvalidManifest, err)
	}
	return string(encoded), nil
}

// ValidateRestartPolicy validates restart_policy and rollout globs on a job manifest.
func ValidateRestartPolicy(jobName string, manifest Manifest) error {
	...
	if len(manifest.RestartGlobs) > 0 && len(manifest.ReloadGlobs) > 0 {
		return fmt.Errorf(
			"%w: job %s restart_globs and reload_globs are mutually exclusive",
			bucket.ErrInvalidManifest, jobName,
		)
	}
	if err := ValidateRestartGlobs(jobName, manifest); err != nil {
		return err
	}
	return ValidateReloadGlobs(jobName, manifest)
}
...
// ValidateReloadGlobs validates reload_globs on a job manifest.
func ValidateReloadGlobs(jobName string, manifest Manifest) error {
	if len(manifest.ReloadGlobs) == 0 {
		return nil
	}
	...
	if policy != RestartPolicyAlways {
		return fmt.Errorf(
			"%w: job %s reload_globs requires restart_policy always",
			bucket.ErrInvalidManifest, jobName,
		)
	}

	for _, pattern := range manifest.ReloadGlobs {
		if err := pathmatch.ValidatePattern(pattern); err != nil {
			return fmt.Errorf("%w: job %s reload_globs: %w", bucket.ErrInvalidManifest, jobName, err)
		}
	}
	return nil
}
