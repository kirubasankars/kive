...
// WorkerRecord is the on-disk workers.conf shape (position omitted => array index).
...
// FormatMemoryMB formats megabytes for workers.conf.
func FormatMemoryMB(mb float64) string {
	return fmt.Sprintf("%d mb", int(math.Round(mb)))
}

// FormatCPUMHz formats megahertz for workers.conf.
func FormatCPUMHz(mhz float64) string {
	return fmt.Sprintf("%d mhz", int(math.Round(mhz)))
}

// ReadWorkersFile loads workers.conf without normalizing labels or defaults.
func ReadWorkersFile() ([]WorkerRecord, error) {
	if err := checkWorkersLegacyJSON(); err != nil {
		return nil, err
	}
	data, err := conf.ReadBytes(workersConfPath())
	if err != nil {
	...
	workers, err := ParseWorkersConf(workersConfPath(), data)
	if err != nil {
...
// WriteWorkersFile writes workers.conf with stable field ordering.
func WriteWorkersFile(workers []WorkerRecord) error {
	data, err := MarshalWorkersFile(workers)
	if err != nil {
		return err
	}
	return os.WriteFile(workersConfPath(), data, 0o644)
}

// MarshalWorkersFile returns workers.conf content with stable field ordering.
func MarshalWorkersFile(workers []WorkerRecord) ([]byte, error) {
	return []byte(EmitWorkersConf(workers)), nil
}

// MergeWorkerFacts returns a copy of workers with memory/cpu updated for matching hosts.
func MergeWorkerFacts(workers []WorkerRecord, updates map[string]WorkerFacts) []WorkerRecord {
	merged := make([]WorkerRecord, len(workers))
	copy(merged, workers)

	for idx := range merged {
		host := strings.TrimSpace(merged[idx].Host)
		facts, ok := updates[host]
		if !ok {
			continue
		}
		merged[idx].Memory = FormatMemoryMB(facts.MemoryMB)
		merged[idx].CPU = FormatCPUMHz(facts.CPUMHz)
	}
	return merged
}
...
func ApplyWorkerFacts(updates map[string]WorkerFacts) ([]WorkerFactsChange, error) {
	...
	if len(workers) == 0 {
		return nil, fmt.Errorf("%w: workers.conf is empty", bucket.ErrInvalidWorkerJSON)
	}
