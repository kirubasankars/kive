...
// ListCertMetrics returns CA and job leaf certificates from KV (same source as kive cat certs).
func ListCertMetrics(tx *sql.Tx, jobsFilter, workersFilter []string) ([]Metric, error) {
	settings, err := bucket.LoadBucketSettings()
	if err != nil {
		return nil, err
	}
	return listCertMetrics(tx, jobsFilter, workersFilter, settings.CertsRenewalBuffer)
}
...
func listCertMetrics(
	...
	rows, err := tx.Query(`
		...
		WHERE deleted = 0
		  ...
		  AND namespace LIKE 'kive/job/%/worker/%'
		  AND version = (
	...
	sort.SliceStable(metrics, func(i, j int) bool {
		if metrics[i].DaysLeft != metrics[j].DaysLeft {
			return metrics[i].DaysLeft < metrics[j].DaysLeft
		}
		if !metrics[i].NotAfter.Equal(metrics[j].NotAfter) {
			return metrics[i].NotAfter.Before(metrics[j].NotAfter)
		}
		if metrics[i].Job != metrics[j].Job {
			return metrics[i].Job < metrics[j].Job
		}
		if metrics[i].WorkerIP != metrics[j].WorkerIP {
			return metrics[i].WorkerIP < metrics[j].WorkerIP
		}
		return metrics[i].CertName < metrics[j].CertName
	})
...
func metricFromPEM(
	...
	return Metric{
		...
		DaysToRenewal: certDaysToRenewal(notAfter, renewalBufferDays, now),
...
// certDaysToRenewal is whole days until the renewal window opens (certs_renewal_buffer
// before NotAfter). Zero or negative means renewal is already due. When the buffer is
// 0, renewal is at expiry so this matches days left until NotAfter.
func certDaysToRenewal(notAfter time.Time, renewalBufferDays int, now time.Time) int {
	notAfter = notAfter.UTC()
	now = now.UTC()
	if renewalBufferDays <= 0 {
		return certDaysLeft(notAfter, now)
	}
	renewalStart := notAfter.Add(-time.Duration(renewalBufferDays) * 24 * time.Hour)
	return int(renewalStart.Sub(now).Hours() / 24)
}
...
// ParseJobWorkerCertNamespace parses kive/job/<job>/worker/<ip> KV namespaces.
func ParseJobWorkerCertNamespace(namespace string) (job, workerIP string, ok bool) {
	return parseJobWorkerCertNamespace(namespace)
}

// CertExpiryStatus returns ok, expiring, or expired using kive renewal buffer semantics.
func CertExpiryStatus(notAfter time.Time, renewalBufferDays int, now time.Time) string {
