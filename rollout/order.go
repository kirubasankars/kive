...
// Package rollout resolves allocation order for batched job operations.
package rollout
...
// ResolvedOrder is the allocation order used for a rollout phase.
...
func readRolloutOrderKV(job string) (string, bool) {
	store := kv.GetKVStore()
	if store == nil {
		return "", false
	}
	item, err := store.Get(jobKVNamespace(job), kv.RolloutOrderKey)
...
func validateRolloutOrderList(job string, orderList []string, activeSet map[string]struct{}) (bool, string) {
	if len(orderList) == 0 && len(activeSet) > 0 {
		return false, "rollout_order is empty"
	}
	seen := make(map[string]struct{}, len(orderList))
	for _, ip := range orderList {
		if _, dup := seen[ip]; dup {
			return false, fmt.Sprintf("rollout_order duplicate %s", ip)
		}
		seen[ip] = struct{}{}
		if _, ok := activeSet[ip]; !ok {
			return false, fmt.Sprintf("rollout_order unknown allocation %s", ip)
		}
	}
	for ip := range activeSet {
		if _, ok := seen[ip]; !ok {
			return false, fmt.Sprintf("rollout_order missing allocation %s", ip)
		}
	}
	return true, ""
}
...
// ResolveOrder picks rollout order for candidates using rollout_order KV when valid.
func ResolveOrder(tx *sql.Tx, job string, candidates []string) (ResolvedOrder, error) {
	if len(candidates) == 0 {
	...
	rawOrder, ok := readRolloutOrderKV(job)
	if !ok {
	...
	valid, reason := validateRolloutOrderList(job, orderList, activeSet)
	if !valid {
		log.Printf("rollout: job %q: %s; using default catalog order", job, reason)
		return ResolvedOrder{
			Ordered:   defaultOrdered,
			FullOrder: strings.Join(defaultOrdered, ","),
			Source:    OrderSourceDefault,
