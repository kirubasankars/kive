...
func ValidateWorkerResources(tx *sql.Tx) error {
	rows, err := tx.Query(`
        ...
        FROM
            allocations a JOIN jobs j ON j.name = a.job
                JOIN
            workers w ON w.worker_ip = a.worker_ip
	...
	for rows.Next() {
		...
		if requiredMemoryMB > 0 && availableMemoryMB <= 0 {
			resourceErrors = append(resourceErrors, fmt.Sprintf(
				"worker_ip %s must specify memory in workers.conf (allocated jobs require %.2f MB)",
		...
		if requiredCPUMHz > 0 && availableCPUMHz <= 0 {
			resourceErrors = append(resourceErrors, fmt.Sprintf(
				"worker_ip %s must specify cpu in workers.conf (allocated jobs require %.2f MHZ)",
...
func ValidateMinAllocationsCount(tx *sql.Tx, jobWorkspace *workspace.DefaultWorkspace) error {
	...
	for _, jobName := range jobNames {
		...
		// Skip until placement produces at least one allocation (e.g. clone before workers match).
		if count == 0 {
			continue
		}
		if count < minCount {
