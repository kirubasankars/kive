...
type portAllocator struct {
	...
	existing    data.PortAssignments
	usedNumbers map[int]struct{}
}

func newPortAllocator(existing data.PortAssignments, portRange bucket.PortRange) *portAllocator {
	if existing == nil {
		existing = make(data.PortAssignments)
	}
...
func (a *portAllocator) assignProvisioned(jobName string, portName string) (int, error) {
	if err := workspace.ValidateJobPortKey(jobName, portName); err != nil {
		return 0, err
	}

	// Reuse the number already stored in job_ports when it lies in the bucket pool.
	// Numbers outside port_range (e.g. a former fixed port like 9500) are
...
func syncPorts(
	...
	for _, portName := range portNames {
		binding := ports[portName]
		port, err := allocator.resolve(jobName, portName, binding)
		if err != nil {
			return err
		}
		...
		_, err = tx.Exec(
			"INSERT INTO job_ports (job_id, name, port, protocol, exposure) VALUES (?, ?, ?, ?, ?)",
			jobID, portName, port, protocol, exposure,
		)
		if err != nil {
			return bucket.DatabaseError(err)
		}
...
func buildPortAllocator(tx *sql.Tx, workspaceJobNames []string, manifests map[string]workspace.Manifest) (*portAllocator, error) {
	needsPorts := false
	...
	existing, err := data.GetAllPortAssignments(tx)
	if err != nil {
		return nil, err
	}
