...
func BuildWorkers(tx *sql.Tx, jobWorkspace *workspace.DefaultWorkspace) ([]string, error) {
	...
	for _, worker := range workspaceWorkers {
		row := tx.QueryRow("SELECT worker_id FROM workers WHERE worker_ip = ?", worker.Host)
		...
		if workerID == "" {
			workerID, err = data.MintUniquePublicID(bucketID+"|"+worker.Host, func(id string) error {
				var n int
				if qerr := tx.QueryRow(`SELECT COUNT(1) FROM workers WHERE worker_id = ?`, id).Scan(&n); qerr != nil {
		...
		upsertWorkerQuery := "INSERT OR REPLACE INTO workers (worker_id, worker_ip, available_memory_mb, available_cpu_mhz, position) VALUES (?, ?, ?, ?, ?)"
	...
	for _, workerIP := range workersToRemove {
		_, err := tx.Exec("DELETE FROM worker_labels WHERE worker_id IN (SELECT worker_id FROM workers WHERE worker_ip = ?)", workerIP)
		if err != nil {
			return nil, bucket.DatabaseError(err)
		}

		_, err = tx.Exec("DELETE FROM worker_tags WHERE worker_id IN (SELECT worker_id FROM workers WHERE worker_ip = ?)", workerIP)
		if err != nil {
			return nil, bucket.DatabaseError(err)
		}

		_, err = tx.Exec("DELETE FROM workers WHERE worker_ip = ?", workerIP)
		if err != nil {
			return nil, bucket.DatabaseError(err)
		}
