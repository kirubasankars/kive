...
const matchingJobsByLabelsQuery = `
            ...
            WHERE (
                ...
                JOIN workers a ON wl_sub.worker_id = a.worker_id
...
func BuildAllocations(tx *sql.Tx, jobWorkspace *workspace.DefaultWorkspace) error {
	...
	for _, workerIP := range workerIPs {
		...
		for rows.Next() {
			...
			if allocationID == "" {
				allocationID, err = data.MintUniquePublicID(bucketID+"|"+jobName+"|"+workerIP, func(id string) error {
					var n int
					if qerr := tx.QueryRow(`SELECT COUNT(1) FROM allocations WHERE alloc_id = ?`, id).Scan(&n); qerr != nil {
			...
			upsertAllocationQuery := `INSERT OR REPLACE INTO allocations (
				alloc_id, job, worker_ip, disabled, removed, version
			) VALUES (?, ?, ?, ?, ?, ?)`
			_, err = tx.Exec(upsertAllocationQuery, allocationID, jobName, workerIP, 0, 0, targetVersion)
			if err != nil {
				_ = rows.Close()
				return bucket.DatabaseError(err)
			}
	...
	_, err = tx.Exec("UPDATE allocations SET removed = 1 WHERE worker_ip NOT IN (SELECT worker_ip FROM workers)")
	if err != nil {
		return bucket.DatabaseError(err)
	}
